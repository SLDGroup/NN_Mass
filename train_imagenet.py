...
parser.add_argument('-d', '--data', metavar='DIR',default='./datasets/imagenet/ILSVRC/Data/CLS-LOC/',
...
parser.add_argument('--post_fix', default='_v2', type=str, metavar='WEIGHT',
                    help='path to pretrained weight (default: none)')

best_prec1 = 0
file_name='./ckpt/log'+parser.parse_args().post_fix+'.txt'
...
def main():
    ...
    model = model_zoo.mobilenet_v2(width_mult=1.1,inverted_residual_setting = [
                # t, c, n, s
                [1,  16, 2, 1],
                [6,  24, 3, 2],
                [6,  32, 4, 2],
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            logger = Logger(os.path.join(args.checkpoint, 'log'+str(args.post_fix)+'.txt'), title=title, resume=True)
        else:
            print("=> no checkpoint found at '{}'".format(args.resume))
    else:
        logger = Logger(os.path.join(args.checkpoint, 'log'+str(args.post_fix)+'.txt'), title=title)
    ...
    logger.close()
    logger.plot()
    savefig(os.path.join(args.checkpoint, 'log'+str(args.post_fix)+'.eps'))
    writer.close()
...
def save_checkpoint(state, is_best, checkpoint='checkpoint', filename='checkpoint'+str(args.post_fix)+'.pth.tar'):
    global args
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'model_best'+str(args.post_fix)+'.pth.tar'))
